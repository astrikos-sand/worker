...
def execute_node(node: dict, nodes_dict: dict[dict], triggered=False):
    ...
    if ready_for_execution:
        if not all(param in inputs for param in input_slots):
            raise Exception(
                f"All inputs are not available for node {node_id}, available inputs {inputs}, required inputs {input_slots}"
            )
        ...
        special_input_slots = filter(
            lambda slot: slot.get("attachment_type", None)
            == SLOT_ATTACHMENT_TYPE.INPUT,
            special_slots,
        )
        special_output_slots = filter(
            lambda slot: slot.get("attachment_type", None)
            == SLOT_ATTACHMENT_TYPE.OUTPUT,
            special_slots,
        )
        ...
        for special_input in special_input_slots:
            ...
            match speciality:
                ...
                case SLOT_SPECIALITY.SIGNAL:
                    signal_slot = name
                    execution_signal = inputs.get(
                        name, False
                    )  # if signal parameter exists then assign execution signal to that value otherwise always true
                case _:
                    ...
                    if special_input is not None:
                        ...
                        raise Exception(
                            f"Invalid speciality for input slot: speciality: {speciality} name: {name}"
                        )
        ...
        extract_special_outputs = lambda slots: dict(
            (slot.get("name", None), slot.get("speciality", None)) for slot in slots
        )

        functional_output_slots = (
            output_slots if not triggered else delayed_output_slots
        )
        functional_special_output_slots = (
            special_output_slots if not triggered else delayed_special_output_slots
        )
        functional_special_output_slots = extract_special_outputs(
            functional_special_output_slots
        )  # convert into key value pair dict
        ...
        for connection in source_connections:
            ...
            if source_slot in functional_special_output_slots.keys():
                functional_special_output_fn = speciality_output.get(
                    functional_special_output_slots.get(source_slot), lambda: None
                )
            ...
            if output_present:
                with node_dict_lock:
                    nodes_dict.get(target_node_id).setdefault("inputs", {}).update(
                        {target_slot: output}
                    )
